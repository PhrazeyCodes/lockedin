...
export function blankDay() {
  return { meals: [], water: 0, habits: {}, lift: null, tasks: [], blocks: [] };
}
  ...
  try {
    ...
    if (data) {
      const day = { ...blankDay(), meals: data.meals || [], water: data.water || 0,
        habits: data.habits || {}, lift: data.lift, tasks: data.tasks || [], blocks: data.blocks || [] };
...
export async function saveDay(uid, date, day) {
  ...
  const row = {
    ...
    lift: day.lift, tasks: day.tasks, blocks: day.blocks,
...
export async function syncFeed(uid, date, day, profile) {
  ...
  if (habitKeys.length) {
    const done = habitKeys.filter((k) => day.habits[k]).length;
    await upsertFeed(uid, date, "habits", { done, total: habitKeys.length });
  }
  if (day.tasks?.length) {
    const done = day.tasks.filter((x) => x.done).length;
    await upsertFeed(uid, date, "tasks", { done, total: day.tasks.length });
  }
}
