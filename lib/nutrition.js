...
// Surface the server's own message when it sends one; fall back to the status.
...
export async function scanImage(base64, mime) {
  ...
  if (!res.ok) throw await errorFrom(res, "Scan failed");
  return res.json();
}
...
export async function describeFood(text) {
  ...
  if (!res.ok) throw await errorFrom(res, "Analysis failed");
  return res.json();
}
